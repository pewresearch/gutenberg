...
function gutenberg_render_block_visibility_support( $block_content, $block ) {
	...
	if ( is_array( $block_visibility ) && ! empty( $block_visibility ) ) {
		...
		// Process viewport visibility (built-in condition).
		if ( is_array( $viewport_config ) && ! empty( $viewport_config ) ) {
			...
			// If viewport sizes have visibility set to false, apply styles.
			if ( ! empty( $hidden_on ) ) {
		...
		// Process 3rd-party registered conditions.
		$registry = WP_Block_Visibility_Conditions_Registry::get_instance();
		foreach ( $registry->get_registered() as $slug => $condition ) {
			// Skip the built-in 'viewport' — already handled above.
			if ( 'viewport' === $slug ) {
				continue;
			}

			$condition_value = $block_visibility[ $slug ] ?? null;
			if ( null === $condition_value ) {
				continue;
			}

			if ( is_callable( $condition['render_callback'] ) ) {
				$block_content = call_user_func(
					$condition['render_callback'],
					$block_content,
					$block,
					$condition_value
				);

				// If render_callback returned empty, the block is hidden.
				if ( '' === $block_content ) {
					return '';
				}
			}
